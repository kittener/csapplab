...
int isTmax(int x) {
  int temp = x+1;
  return !(temp+temp)&(!!(temp)); // ex 0xffffffff
}
/* 
 ...
 *   Rating: 3
 */
int isAsciiDigit(int x) { // sub bound 
  int LowBound = ~0x30+1;
  int HighBound = ~0x3a+1;
  return (!((x+LowBound)>>31))&!!((x+HighBound)>>31);
}
/* 
...
int isLessOrEqual(int x, int y) {
  ...
  int ysign = y>>31;
  //same sign
  int anssign = y+(~x+1)>>31;
  //other
  int flag = (xsign^ysign)&1;
  //printf("%d %d %d\n",xsign,ysign,flag);
  return ((!flag)&(!anssign&1))|(flag&(xsign>>31));
}
//4
/* 
...
int howManyBits(int x) {
  ...
  shift1 = !!(x>>1);
  x = x>>shift1;
  return shift16+shift8+shift4+shift2+shift1+x+1;//+1......sign......
}
//float
/* 
